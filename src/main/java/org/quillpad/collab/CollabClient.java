...
public class CollabClient {
    ...
    private final StringProperty currentWriteOwner = new SimpleStringProperty();
        ...
        switch (message.getType()) {
            case CONNECT:
                if (message.getSenderId() != null && !message.getSenderId().isBlank()) {
                    Platform.runLater(() -> userId.set(message.getSenderId()));
                }
                break;
    ...
    public void updateDocument(Document document) {
        ...
        if (docId == null || docId.isBlank()) {
            notifyError("Cannot update: no document selected");
            return;
        }

        sendMessage(Message.documentUpdate(userId.get(), docId, document));
    }
    ...
    public void requestWriteAccess(String documentId) {
        if (documentId != null && !documentId.isBlank()) {
            sendMessage(Message.requestWriteAccess(userId.get(), documentId));
        }
    }

    public void releaseWriteAccess(String documentId) {
        if (documentId != null && !documentId.isBlank()) {
            sendMessage(Message.releaseWriteAccess(userId.get(), documentId));
        }
    }

    public void grantWriteAccess(String documentId, String targetUserId) {
        if (documentId != null && !documentId.isBlank() && targetUserId != null && !targetUserId.isBlank()) {
            sendMessage(Message.grantWriteAccess(userId.get(), documentId, targetUserId));
        }
    }
    ...
    public String getCurrentWriteOwner() {
        return currentWriteOwner.get();
    }
